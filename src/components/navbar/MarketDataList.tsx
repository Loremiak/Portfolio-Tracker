...
const MarketDataList: React.FC<{ globalMarketData: GlobalMarketData; isLoading: boolean }> = ({ globalMarketData, isLoading }) => {
    return (
        <List
            sx={{
                display: 'flex',
        ...
        >
            <li>
                Waluty:
                <span> {isLoading ? <CircularProgress size="13px" /> : handleBiggerValues(globalMarketData.active_cryptocurrencies)}</span>
            </li>
            <li>
                Giełdy:
            ...
            <li>
                ...
                <span>{isLoading ? <CircularProgress size="13px" /> : formatNumber(globalMarketData.total_market_cap.usd)}</span> USD
                <Typography
                    component="span"
                    fontWeight="bold"
                    fontSize={{ xs: '0.6rem', sm: '0.8rem', md: '1.2rem' }}
            ...
            <li>Wolumen: {isLoading ? <CircularProgress size="13px" /> : formatNumber(globalMarketData.total_volume.usd)} USD</li>
        </List>
    );
};
