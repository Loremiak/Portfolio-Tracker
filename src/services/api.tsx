...
export function useCoinDetailsInfo(id: string) {
	return useQuery({
		...
		queryFn: async () => {
			const response = await axios.get<CoinDetails>(`${BASE_URL}/coins/${id}`);
			return response.data;
		},
	});
}
...
export function useHistoricalChartData(id: string) {
	return useQuery({
		...
		queryFn: async () => {
			const response = await axios.get<ChartData>(`${BASE_URL}/coins/${id}/market_chart?vs_currency=usd&days=1`);
			return response.data;
		},
	});
}
